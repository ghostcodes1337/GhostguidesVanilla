...
local CONFIG = {
    ...
    colors = {
        even = {0.10, 0.12, 0.22, 1},
        odd = {0.12, 0.14, 0.25, 1},
        active = {0.16, 0.18, 0.32, 1},
        ongoing = {0.18, 0.20, 0.35, 1}
    },
...
GLV_QuestProgressWatcher:SetScript("OnEvent", function()
    if GLV and GLV.RefreshGuide then
        GLV:RefreshGuide()
    end
end)
...
function GLV:CreateGuideSteps(scrollChild, guide, guideId, callback)
    ...
    -- ongoing steps now use floating tracker only
    ongoingIndices = {}
    ...
    for _, ongoingIdx in ipairs(ongoingIndices) do
        ...
        for _, line in ipairs(step.lines) do
            if line.text and line.text ~= "" then
                table.insert(allLines, line.text)
            end
        end

        local stepText = table.concat(allLines, "\n")

            -- manual wrap before creating row
            local wrappedText = ""
            local lineLength = 0
            local lineCount = 1

            for word in string.gfind(stepText, "[^%s]+") do
                local wordLen = string.len(word)

                if lineLength + wordLen > maxCharsPerLine then
                    wrappedText = wrappedText .. "\n" .. word
                    lineLength = wordLen
                    lineCount = lineCount + 1
                else
                    if wrappedText == "" then
                        ...
                        lineLength = lineLength + wordLen + 1
                    end
                end
            end

            -- calculate exact row height BEFORE creating row
            local rowHeight = (lineCount * lineHeight) + 12

            local row = CreateFrame("Frame", nil, GLV_OngoingTracker.content)
            row:SetWidth(300)
            row:SetHeight(rowHeight)
            row:SetPoint("TOPLEFT", GLV_OngoingTracker.content, "TOPLEFT", 0, -currentYOffset)

            -- text
            local text = row:CreateFontString(nil, "OVERLAY", "GameFontNormal")
            text:SetPoint("TOPLEFT", row, "TOPLEFT", 6, -4)
            text:SetWidth(255)
            ...
            check:SetWidth(18)
            check:SetHeight(18)
            check:SetPoint("TOPRIGHT", row, "TOPRIGHT", -4, -6)
            ...
            check:SetScript("OnClick", function()
                ...
                local origIdx = GLV.CurrentDisplayToOriginal[ongoingIdx]
                if origIdx then
                ...
                if check:GetChecked() then
                    GLV.OngoingStepsManager:Deactivate(ongoingIdx)
                end

                GLV:RefreshGuide()
            end)

            -- reserve exact space for next block
            currentYOffset = currentYOffset + rowHeight + 4
    ...
    for idx, step in ipairs(displaySteps) do
        ...
        -- ongoing steps are shown only in floating tracker
        if step.ongoing then
            skipThisStep = true
        end
        ...
        if idx == activeStep then
            ...
            -- update floating ongoing tracker
            local ongoingText = nil
            local ongoingLines = {}
            ...
            if ongoingIndices and table.getn(ongoingIndices) > 0 then
                for _, ongoingIdx in ipairs(ongoingIndices) do
                    ...
                    if step and step.lines then
                        for _, line in ipairs(step.lines) do
                            if line.text and line.text ~= "" then
                                table.insert(ongoingLines, line.text)
                            end
                        end
                    end
                end
            ...
            if table.getn(ongoingLines) > 0 then
                ongoingText = table.concat(ongoingLines, "\n")
            end

            GLV_UpdateOngoingTracker(displaySteps, ongoingIndices, currentGuideId)
        else
        ...
        stepHeaderText:SetPoint("TOPLEFT", frame, "TOPLEFT", 8, -4)
        stepHeaderText:SetJustifyH("LEFT")
        stepHeaderText:SetTextColor(1, 1, 1)
        stepHeaderText:SetText("Step " .. idx)

        local headerHeight = 16
        local yOffset = -(headerHeight + 4)
        local frameHeight = headerHeight + 4
        local lineStrings = {}
        ...
        for li, line in ipairs(step.lines) do
            ...
            -- register quest progress tracker for live objective updates
